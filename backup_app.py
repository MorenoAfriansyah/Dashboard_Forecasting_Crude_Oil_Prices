...
def load_resources():
    ...
    try:
        ...
        if os.path.exists('best_params_case_5.json'):
            with open('best_params_case_5.json', 'r') as f:
                params = json.load(f)
            window_size = params.get('sliding_window', 30)
            
        # Load Model
        if os.path.exists('best_model_case_5.h5'):
            model = tf.keras.models.load_model("best_model_case_5.h5", compile=False)
        else:
            return None, None, None, None

        # Load Scalers
        if os.path.exists('scaler_X_case_5.pkl') and os.path.exists('scaler_y_case_5.pkl'):
            scaler_X = joblib.load("scaler_X_case_5.pkl")
            scaler_y = joblib.load("scaler_y_case_5.pkl")
        else:
            return None, None, None, None
